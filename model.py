...
class AzzuNet:
    ...
    def global_max_pool(self, name, local_f):
        with tf.variable_scope(name):
            global_features = tf.reduce_max(local_f, reduction_indices=[1], name="global_features")
    ...
    def __init__(self, input_length, embedding_length_word, embedding_length_dep):
        ...
        with tf.variable_scope('AzzuNet'):
            with tf.variable_scope('variables'):
                ...
                w_f = tf.Variable(tf.random_normal([200, 19]), dtype=tf.float32, name="w_f")
                b_f = tf.Variable(tf.ones([19]), dtype=tf.float32, name="b_f")
                w_c = tf.Variable(tf.random_normal([400, 10]), dtype=tf.float32, name="w_c")
