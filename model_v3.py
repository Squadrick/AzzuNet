    ...
    def __init__(self, input_length, embedding_length_word, embedding_length_dep):
        ...
        with tf.variable_scope('AzzuNet'):
            ...
            with tf.variable_scope('inputs'):
                self.words = tf.placeholder(shape = [None, input_length, embedding_length_word], dtype = tf.float32,
                                            name = 'words')
                self.deps = tf.placeholder(shape = [None, input_length - 1, embedding_length_dep], dtype = tf.float32,
                                           name = 'dependencies')
            ...
            with tf.variable_scope('convolution_network'):
                local_features_f = self.cnn_block('forward', w_con_f, b_con_f, lstm_word_f, lstm_deps_f,
                                                  input_length - 1)
                local_features_b = self.cnn_block('backward', w_con_b, b_con_b, lstm_word_b, lstm_deps_b,
                                                  input_length - 1)
